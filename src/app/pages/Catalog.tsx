...
export function Catalog() {
  const { items, addItem, deleteItem, updateItem } = useApp();
  ...
  const openAdd = () => { setForm(emptyForm()); setEditingItem(null); setFormErrors({}); setShowModal(true); };
  const openEdit = (item: ClothingItem) => { setForm({ ...item }); setEditingItem(item); setFormErrors({}); setShowModal(true); };
    ...
    <div style={{ background: '#0A0A0A', minHeight: '100vh', fontFamily: "'Space Grotesk', sans-serif" }} className="p-6 lg:p-10">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px', marginTop: '4px' }}>
            {filtered.length} items {catFilter && `in ${catFilter.toUpperCase()}`}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => { setSearchParams({}); setPage(1); trackPreferredCategory('all'); }}
          ...
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => { setSearchParams({ category: cat }); setPage(1); trackPreferredCategory(cat); }}
              className="px-3 py-2 rounded text-xs"
                ...
                {paged.map((item, i) => (
                  <motion.tr key={item.id}
                    ...
                    <td className="px-4 py-3">
                      <img src={item.imageUrl} alt={item.name} className="rounded object-cover"
                    ...
                    <td className="px-4 py-3">
                      <Link to={`/catalog/${item.id}`}>
                        ...
                        {item.featured && (
                          <span className="ml-2" style={{ background: '#FFE500', color: '#0A0A0A', fontSize: '8px', fontWeight: 700, padding: '1px 5px', letterSpacing: '0.1em' }}>
                            HOT
                          </span>
                        )}
                      </Link>
                    ...
                    <td className="px-4 py-3">
                      <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '11px', letterSpacing: '0.1em' }}>
                        {item.category.toUpperCase()}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span style={{ color: '#FFE500', fontFamily: "'Orbitron', sans-serif", fontSize: '14px', fontWeight: 700 }}>
                        ${item.price}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span style={{ color: item.stock > 20 ? '#00E5FF' : item.stock > 5 ? '#FFE500' : '#FF4444', fontSize: '13px', fontWeight: 600 }}>
                        {item.stock}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                        ...
                        {deleteConfirm === item.id ? (
                          <div className="flex items-center gap-1">
                            <button onClick={() => { deleteItem(item.id); setDeleteConfirm(null); }}
                              className="p-1.5 rounded" style={{ color: '#FF4444', background: 'rgba(255,68,68,0.1)' }}>
                              <Check size={14} />
                            </button>
                            <button onClick={() => setDeleteConfirm(null)}
                              className="p-1.5 rounded" style={{ color: 'rgba(255,255,255,0.4)' }}>
                              <X size={14} />
                            </button>
                          </div>
                        ) : (
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              ...
              style={{ color: page === 1 ? 'rgba(255,255,255,0.2)' : '#FFE500', border: '1px solid rgba(255,229,0,0.2)', background: 'transparent' }}>
              <ChevronLeft size={16} />
            </button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => setPage(p)}
                ...
                style={{
                  background: page === p ? '#FFE500' : 'transparent',
                  color: page === p ? '#0A0A0A' : 'rgba(255,255,255,0.5)',
            ...
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
              className="p-2 rounded transition-colors"
              style={{ color: page === totalPages ? 'rgba(255,255,255,0.2)' : '#FFE500', border: '1px solid rgba(255,229,0,0.2)', background: 'transparent' }}>
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input type={type} placeholder={placeholder}
                      ...
                      style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,229,0,0.15)', color: '#FFFFFF', fontSize: '13px' }} />
                    {formErrors[key] && <p style={{ color: '#FF4444', fontSize: '10px', marginTop: '2px' }}>{formErrors[key]}</p>}
                  </div>
                ))}
